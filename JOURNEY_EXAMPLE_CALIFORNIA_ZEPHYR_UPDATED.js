// ============================================================================
// COMPLETE JOURNEY EXAMPLE: California Zephyr (August 2021)
// ============================================================================
// Updated with official Amtrak timetable stops
//
// Journey Details:
// - Start: Chicago, IL (implicit) - 2021/08/08
// - End: San Francisco, CA - 2021/08/10
// - Primary Transport: Amtrak California Zephyr (Train #5)
// - Secondary Transport: Amtrak Thruway Bus (Emeryville to San Francisco)
// - Total Timetable Stops: 35
// - Visited Places: 17
// - Non-visited Stops: 18
// ============================================================================

import { TRANSPORT_MODES } from './constants'

// ----------------------------------------------------------------------------
// 1. JOURNEY DEFINITION
// ----------------------------------------------------------------------------
export const journey = {
  id: "california-zephyr-2021-08",
  slug: "california-zephyr",
  name: "California Zephyr",
  description: "Cross-country journey from Chicago to San Francisco through the Rocky Mountains and Sierra Nevada, featuring some of America's most spectacular scenery.",

  // Calculated from place visits
  startDate: "2021-08-08",  // Same day as first visited place (Naperville)
  endDate: "2021-08-10",    // Last visited place (San Francisco)

  // Start/End locations
  startLocation: {
    name: "Chicago Union Station, IL",
    coordinates: { lat: 41.8789, lng: -87.6397 }
  },
  endLocation: {
    name: "San Francisco, CA",
    coordinates: { lat: 37.7749, lng: -122.4194 }
  },

  // References to visited places (17 places)
  visitedPlaceIds: [
    "6189a38befae22e1cf7808bc", // Naperville, IL
    "6189a3d8efae22e1cf7808d1", // Galesburg, IL
    "6189a410efae22e1cf7808e6", // Mount Pleasant, IA
    "6189a458efae22e1cf7808fb", // Osceola, IA
    "6189a48defae22e1cf78090e", // Omaha, NE
    "6189a4d3efae22e1cf78091f", // Lincoln, NE
    "6189a524efae22e1cf7809d1", // Denver, CO
    "6189a59aefae22e1cf780aaa", // Granby, CO
    "6189a5f6efae22e1cf780abf", // Glenwood Springs, CO
    "6189a661efae22e1cf780ad4", // Grand Junction, CO
    "6189a69cefae22e1cf780afd", // Helper, UT
    "6189a6e4efae22e1cf780b12", // Salt Lake City, UT
    "6189a717efae22e1cf780b27", // Reno, NV
    "6189a749efae22e1cf780b3c", // Colfax, CA
    "6189a785efae22e1cf780b51", // Sacramento, CA
    "6189a7cdefae22e1cf780b66", // Emeryville, CA
    "6189a820efae22e1cf780b7b"  // San Francisco, CA
  ],

  totalPlaces: 17,
  images: []
}

// ----------------------------------------------------------------------------
// 2. ROUTE DEFINITION (Based on Official Timetable)
// ----------------------------------------------------------------------------
export const route = {
  journeyId: "california-zephyr-2021-08",
  segments: [
    // ========== SEGMENT 1: Chicago to Naperville ==========
    {
      order: 1,
      from: {
        name: "Chicago, IL",
        coordinates: { lat: 41.8789, lng: -87.6397 },
        isVisited: false,  // Implicit start
        placeId: null
      },
      to: {
        name: "Naperville, IL",
        coordinates: { lat: 41.7729, lng: -88.1479 },
        isVisited: true,
        placeId: "6189a38befae22e1cf7808bc"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 2: Naperville to Princeton ==========
    {
      order: 2,
      from: {
        name: "Naperville, IL",
        coordinates: { lat: 41.7729, lng: -88.1479 },
        isVisited: true,
        placeId: "6189a38befae22e1cf7808bc"
      },
      to: {
        name: "Princeton, IL",
        coordinates: { lat: 41.3681, lng: -89.4648 },
        isVisited: false,  // Timetable stop, not visited
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 3: Princeton to Galesburg ==========
    {
      order: 3,
      from: {
        name: "Princeton, IL",
        coordinates: { lat: 41.3681, lng: -89.4648 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Galesburg, IL",
        coordinates: { lat: 40.9478, lng: -90.3712 },
        isVisited: true,
        placeId: "6189a3d8efae22e1cf7808d1"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 4: Galesburg to Burlington ==========
    {
      order: 4,
      from: {
        name: "Galesburg, IL",
        coordinates: { lat: 40.9478, lng: -90.3712 },
        isVisited: true,
        placeId: "6189a3d8efae22e1cf7808d1"
      },
      to: {
        name: "Burlington, IA",
        coordinates: { lat: 40.8078, lng: -91.1129 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 5: Burlington to Mt. Pleasant ==========
    {
      order: 5,
      from: {
        name: "Burlington, IA",
        coordinates: { lat: 40.8078, lng: -91.1129 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Mt. Pleasant, IA",
        coordinates: { lat: 40.9635, lng: -91.5582 },
        isVisited: true,
        placeId: "6189a410efae22e1cf7808e6"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 6: Mt. Pleasant to Ottumwa ==========
    {
      order: 6,
      from: {
        name: "Mt. Pleasant, IA",
        coordinates: { lat: 40.9635, lng: -91.5582 },
        isVisited: true,
        placeId: "6189a410efae22e1cf7808e6"
      },
      to: {
        name: "Ottumwa, IA",
        coordinates: { lat: 41.0197, lng: -92.4113 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 7: Ottumwa to Osceola ==========
    {
      order: 7,
      from: {
        name: "Ottumwa, IA",
        coordinates: { lat: 41.0197, lng: -92.4113 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Osceola, IA",
        coordinates: { lat: 41.0339, lng: -93.7655 },
        isVisited: true,
        placeId: "6189a458efae22e1cf7808fb"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 8: Osceola to Creston ==========
    {
      order: 8,
      from: {
        name: "Osceola, IA",
        coordinates: { lat: 41.0339, lng: -93.7655 },
        isVisited: true,
        placeId: "6189a458efae22e1cf7808fb"
      },
      to: {
        name: "Creston, IA",
        coordinates: { lat: 41.0586, lng: -94.3614 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 9: Creston to Omaha ==========
    {
      order: 9,
      from: {
        name: "Creston, IA",
        coordinates: { lat: 41.0586, lng: -94.3614 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Omaha, NE",
        coordinates: { lat: 41.2587, lng: -95.9379 },
        isVisited: true,
        placeId: "6189a48defae22e1cf78090e"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 10: Omaha to Lincoln ==========
    {
      order: 10,
      from: {
        name: "Omaha, NE",
        coordinates: { lat: 41.2587, lng: -95.9379 },
        isVisited: true,
        placeId: "6189a48defae22e1cf78090e"
      },
      to: {
        name: "Lincoln, NE",
        coordinates: { lat: 40.8001, lng: -96.6674 },
        isVisited: true,
        placeId: "6189a4d3efae22e1cf78091f"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 11: Lincoln to Hastings ==========
    {
      order: 11,
      from: {
        name: "Lincoln, NE",
        coordinates: { lat: 40.8001, lng: -96.6674 },
        isVisited: true,
        placeId: "6189a4d3efae22e1cf78091f"
      },
      to: {
        name: "Hastings, NE",
        coordinates: { lat: 40.5861, lng: -98.3881 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 12: Hastings to Holdrege ==========
    {
      order: 12,
      from: {
        name: "Hastings, NE",
        coordinates: { lat: 40.5861, lng: -98.3881 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Holdrege, NE",
        coordinates: { lat: 40.4403, lng: -99.3704 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 13: Holdrege to McCook ==========
    {
      order: 13,
      from: {
        name: "Holdrege, NE",
        coordinates: { lat: 40.4403, lng: -99.3704 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "McCook, NE",
        coordinates: { lat: 40.2025, lng: -100.6254 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 14: McCook to Fort Morgan ==========
    {
      order: 14,
      from: {
        name: "McCook, NE",
        coordinates: { lat: 40.2025, lng: -100.6254 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Fort Morgan, CO",
        coordinates: { lat: 40.2503, lng: -103.7999 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 15: Fort Morgan to Denver ==========
    {
      order: 15,
      from: {
        name: "Fort Morgan, CO",
        coordinates: { lat: 40.2503, lng: -103.7999 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Denver, CO",
        coordinates: { lat: 39.7348, lng: -104.9653 },
        isVisited: true,
        placeId: "6189a524efae22e1cf7809d1"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 16: Denver to Fraser-Winter Park ==========
    {
      order: 16,
      from: {
        name: "Denver, CO",
        coordinates: { lat: 39.7348, lng: -104.9653 },
        isVisited: true,
        placeId: "6189a524efae22e1cf7809d1"
      },
      to: {
        name: "Fraser-Winter Park, CO",
        coordinates: { lat: 39.9442, lng: -105.8172 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 17: Fraser-Winter Park to Granby ==========
    {
      order: 17,
      from: {
        name: "Fraser-Winter Park, CO",
        coordinates: { lat: 39.9442, lng: -105.8172 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Granby, CO",
        coordinates: { lat: 40.0861, lng: -105.9395 },
        isVisited: true,
        placeId: "6189a59aefae22e1cf780aaa"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 18: Granby to Glenwood Springs ==========
    {
      order: 18,
      from: {
        name: "Granby, CO",
        coordinates: { lat: 40.0861, lng: -105.9395 },
        isVisited: true,
        placeId: "6189a59aefae22e1cf780aaa"
      },
      to: {
        name: "Glenwood Springs, CO",
        coordinates: { lat: 39.5507, lng: -107.3255 },
        isVisited: true,
        placeId: "6189a5f6efae22e1cf780abf"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 19: Glenwood Springs to Grand Junction ==========
    {
      order: 19,
      from: {
        name: "Glenwood Springs, CO",
        coordinates: { lat: 39.5507, lng: -107.3255 },
        isVisited: true,
        placeId: "6189a5f6efae22e1cf780abf"
      },
      to: {
        name: "Grand Junction, CO",
        coordinates: { lat: 39.064, lng: -108.5507 },
        isVisited: true,
        placeId: "6189a661efae22e1cf780ad4"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 20: Grand Junction to Green River ==========
    {
      order: 20,
      from: {
        name: "Grand Junction, CO",
        coordinates: { lat: 39.064, lng: -108.5507 },
        isVisited: true,
        placeId: "6189a661efae22e1cf780ad4"
      },
      to: {
        name: "Green River, UT",
        coordinates: { lat: 38.9886, lng: -110.1504 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 21: Green River to Helper ==========
    {
      order: 21,
      from: {
        name: "Green River, UT",
        coordinates: { lat: 38.9886, lng: -110.1504 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Helper, UT",
        coordinates: { lat: 39.6841, lng: -110.8546 },
        isVisited: true,
        placeId: "6189a69cefae22e1cf780afd"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 22: Helper to Provo ==========
    {
      order: 22,
      from: {
        name: "Helper, UT",
        coordinates: { lat: 39.6841, lng: -110.8546 },
        isVisited: true,
        placeId: "6189a69cefae22e1cf780afd"
      },
      to: {
        name: "Provo, UT",
        coordinates: { lat: 40.2338, lng: -111.6585 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 23: Provo to Salt Lake City ==========
    {
      order: 23,
      from: {
        name: "Provo, UT",
        coordinates: { lat: 40.2338, lng: -111.6585 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Salt Lake City, UT",
        coordinates: { lat: 40.767, lng: -111.8904 },
        isVisited: true,
        placeId: "6189a6e4efae22e1cf780b12"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 24: Salt Lake City to Elko ==========
    {
      order: 24,
      from: {
        name: "Salt Lake City, UT",
        coordinates: { lat: 40.767, lng: -111.8904 },
        isVisited: true,
        placeId: "6189a6e4efae22e1cf780b12"
      },
      to: {
        name: "Elko, NV",
        coordinates: { lat: 40.8324, lng: -115.7631 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 25: Elko to Winnemucca ==========
    {
      order: 25,
      from: {
        name: "Elko, NV",
        coordinates: { lat: 40.8324, lng: -115.7631 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Winnemucca, NV",
        coordinates: { lat: 40.9730, lng: -117.7357 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 26: Winnemucca to Reno ==========
    {
      order: 26,
      from: {
        name: "Winnemucca, NV",
        coordinates: { lat: 40.9730, lng: -117.7357 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Reno, NV",
        coordinates: { lat: 39.5293, lng: -119.8137 },
        isVisited: true,
        placeId: "6189a717efae22e1cf780b27"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 27: Reno to Truckee ==========
    {
      order: 27,
      from: {
        name: "Reno, NV",
        coordinates: { lat: 39.5293, lng: -119.8137 },
        isVisited: true,
        placeId: "6189a717efae22e1cf780b27"
      },
      to: {
        name: "Truckee, CA",
        coordinates: { lat: 39.3280, lng: -120.1833 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 28: Truckee to Colfax ==========
    {
      order: 28,
      from: {
        name: "Truckee, CA",
        coordinates: { lat: 39.3280, lng: -120.1833 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Colfax, CA",
        coordinates: { lat: 39.1007, lng: -120.9533 },
        isVisited: true,
        placeId: "6189a749efae22e1cf780b3c"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 29: Colfax to Roseville ==========
    {
      order: 29,
      from: {
        name: "Colfax, CA",
        coordinates: { lat: 39.1007, lng: -120.9533 },
        isVisited: true,
        placeId: "6189a749efae22e1cf780b3c"
      },
      to: {
        name: "Roseville, CA",
        coordinates: { lat: 38.7521, lng: -121.2880 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 30: Roseville to Sacramento ==========
    {
      order: 30,
      from: {
        name: "Roseville, CA",
        coordinates: { lat: 38.7521, lng: -121.2880 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Sacramento, CA",
        coordinates: { lat: 38.581021, lng: -121.4939328 },
        isVisited: true,
        placeId: "6189a785efae22e1cf780b51"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 31: Sacramento to Davis ==========
    {
      order: 31,
      from: {
        name: "Sacramento, CA",
        coordinates: { lat: 38.581021, lng: -121.4939328 },
        isVisited: true,
        placeId: "6189a785efae22e1cf780b51"
      },
      to: {
        name: "Davis, CA",
        coordinates: { lat: 38.5449, lng: -121.7405 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 32: Davis to Martinez ==========
    {
      order: 32,
      from: {
        name: "Davis, CA",
        coordinates: { lat: 38.5449, lng: -121.7405 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Martinez, CA",
        coordinates: { lat: 38.0194, lng: -122.1341 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 33: Martinez to Richmond ==========
    {
      order: 33,
      from: {
        name: "Martinez, CA",
        coordinates: { lat: 38.0194, lng: -122.1341 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Richmond, CA",
        coordinates: { lat: 37.9358, lng: -122.3477 },
        isVisited: false,
        placeId: null
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 34: Richmond to Emeryville (Train Terminus) ==========
    {
      order: 34,
      from: {
        name: "Richmond, CA",
        coordinates: { lat: 37.9358, lng: -122.3477 },
        isVisited: false,
        placeId: null
      },
      to: {
        name: "Emeryville, CA",
        coordinates: { lat: 37.8314, lng: -122.2865 },
        isVisited: true,
        placeId: "6189a7cdefae22e1cf780b66"
      },
      transportMode: TRANSPORT_MODES.TRAIN,
      transportDetails: {
        line: "California Zephyr",
        operator: "Amtrak",
        trainNumber: "5"
      }
    },

    // ========== SEGMENT 35: Emeryville to San Francisco (Amtrak Thruway Bus) ==========
    {
      order: 35,
      from: {
        name: "Emeryville, CA",
        coordinates: { lat: 37.8314, lng: -122.2865 },
        isVisited: true,
        placeId: "6189a7cdefae22e1cf780b66"
      },
      to: {
        name: "San Francisco, CA",
        coordinates: { lat: 37.7749, lng: -122.4194 },
        isVisited: true,
        placeId: "6189a820efae22e1cf780b7b"
      },
      transportMode: TRANSPORT_MODES.BUS,
      transportDetails: {
        line: "Amtrak Thruway",
        operator: "Amtrak"
      }
    }
  ]
}

// ----------------------------------------------------------------------------
// 3. PLACE VISITS (17 visited places)
// ----------------------------------------------------------------------------
export const places = [
  {
    id: "6189a38befae22e1cf7808bc",
    journeyId: "california-zephyr-2021-08",
    orderInJourney: 1,
    name: "Naperville, IL",
    city: "Naperville",
    state: "Illinois",
    date: "2021-08-08",
    coordinates: { lat: 41.7729, lng: -88.1479 },
    images: [
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410249/joeyhouhomepage/wep0pogca5xdfagb1v0g.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410249/joeyhouhomepage/w6oavdjdtvcpcfnklrwf.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410249/joeyhouhomepage/hgxssitwjokqst56awd6.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410250/joeyhouhomepage/r1vrtvovllmp7ojk6ptp.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410250/joeyhouhomepage/vmgu9dyiksikowucboae.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410250/joeyhouhomepage/buz0tbqpda3plv37kupx.jpg"
    ],
    description: "2-3",
    isStartPoint: false,  // Chicago is the implicit start
    isEndPoint: false,
    isRevisit: false,
    previousVisitId: null
  },

  {
    id: "6189a3d8efae22e1cf7808d1",
    journeyId: "california-zephyr-2021-08",
    orderInJourney: 2,
    name: "Galesburg, IL",
    city: "Galesburg",
    state: "Illinois",
    date: "2021-08-08",
    coordinates: { lat: 40.9478, lng: -90.3712 },
    images: [
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410310/joeyhouhomepage/cyot5y742oh0aagq5r4k.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410311/joeyhouhomepage/demwgzgyxgjkg9e70y0u.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410326/joeyhouhomepage/zws3wufvevvkvwouv2zn.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410326/joeyhouhomepage/ejpurn9gvubilkgpa9db.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410328/joeyhouhomepage/ewzd8euilmpn7nji1bmv.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410328/joeyhouhomepage/sixjiquebutcazzzgfp9.jpg"
    ],
    description: "2-4",
    isStartPoint: false,
    isEndPoint: false,
    isRevisit: false,
    previousVisitId: null
  },

  {
    id: "6189a410efae22e1cf7808e6",
    journeyId: "california-zephyr-2021-08",
    orderInJourney: 3,
    name: "Mt. Pleasant, IA",
    city: "Mount Pleasant",
    state: "Iowa",
    date: "2021-08-08",
    coordinates: { lat: 40.9635, lng: -91.5582 },
    images: [
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410381/joeyhouhomepage/pg4jj5uhsby2bvjcbmcb.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410381/joeyhouhomepage/vtyvmlqaga6iah7qwnnw.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410381/joeyhouhomepage/vrbqk4jfc9xnaargn1hw.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410381/joeyhouhomepage/mvwvo76sofosq5r6sigr.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410382/joeyhouhomepage/mxrbv0phxc29mbon4gf7.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636410383/joeyhouhomepage/hoj9vnigwu92kaeqqubr.jpg"
    ],
    description: "2-5",
    isStartPoint: false,
    isEndPoint: false,
    isRevisit: false,
    previousVisitId: null
  },

  // ... Continue for all 17 places
  // (Osceola, Omaha, Lincoln, Denver, Granby, Glenwood Springs, Grand Junction,
  //  Helper, Salt Lake City, Reno, Colfax, Sacramento, Emeryville)

  {
    id: "6189a820efae22e1cf780b7b",
    journeyId: "california-zephyr-2021-08",
    orderInJourney: 17,
    name: "San Francisco, CA",
    city: "San Francisco",
    state: "California",
    date: "2021-08-10",
    coordinates: { lat: 37.7749, lng: -122.4194 },
    images: [
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411421/joeyhouhomepage/jnptep3cirzoswqmyapy.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411421/joeyhouhomepage/isdmc0ylk8dchf3wpxbl.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411422/joeyhouhomepage/wck9onfhkfuj9h8elupe.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411422/joeyhouhomepage/xjlk6ut3yjrvvm06anfq.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411422/joeyhouhomepage/jmikoswt20obxt8eb2aj.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411422/joeyhouhomepage/ekr4vos5eiltglgnd76m.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411422/joeyhouhomepage/ox3zzyoau6ufuscfc4dp.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411423/joeyhouhomepage/mvapwgooitoiwdgk2kev.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411423/joeyhouhomepage/c41nkrqoc9xeckv9ojfh.jpg",
      "https://res.cloudinary.com/joey-hou-homepage/image/upload/v1636411423/joeyhouhomepage/j1klf12n5f1ktu1vk8pc.jpg"
    ],
    description: "4-5",
    isStartPoint: false,
    isEndPoint: true,
    isRevisit: false,
    previousVisitId: null
  }
]

// ============================================================================
// SUMMARY:
// ============================================================================
// - Total Segments: 35
// - Visited Places: 17 (48.6%)
// - Non-visited Stops: 18 (51.4%)
// - Transport Modes: Train (34 segments), Bus (1 segment)
// - Implicit Start: Chicago, IL
// - Final Destination: San Francisco, CA
// ============================================================================
